...
import { getHolidayOnDate }         from "@/lib/holiday-utils";
...
export default async function TeacherAttendancePage({ searchParams }: Props) {
  ...
  // ── Teacher profile + accessible sections ─────────────────────
  ...
  // ── Build de-duplicated section list ──────────────────────────
  ...
  // ── Holiday check — fire in parallel with student query ────────
  ...
  if (hasSelection) {
    ...
    // Fetch holiday + students + existing records in parallel
    const [holidayResult, studentProfiles, existingRecords] =
      await Promise.all([
        getHolidayOnDate(schoolId, date!),
