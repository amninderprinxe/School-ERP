  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/school-admin/fees/structures/new"
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {structures.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">No fee structures yet</p>
            <p className="text-xs text-gray-400 mt-1">
              Create a fee structure to define how much students owe.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["Category", "Class", "Year", "Amount", "Due Date", "Assigned", ""].map((h) => (
                  ...
                  <tr key={s.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-gray-900">{s.feeCategory.name}</p>
                          {s.description && (
                            <p className="text-xs text-gray-400 truncate max-w-[160px]">{s.description}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-sm font-mono text-gray-600">{s.academicYear}</td>
                    <td className="px-5 py-4 font-semibold text-gray-900">{fmtCurrency(s.amount)}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        {s.dueDate && <CalendarDays className="w-3.5 h-3.5 text-gray-400" />}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-1 text-xs font-medium bg-gray-100 text-gray-600 rounded-full">
                          {s._count.payments} student{s._count.payments !== 1 ? "s" : ""}
                        </span>
                        <AssignButton
                          structureId={s.id}
