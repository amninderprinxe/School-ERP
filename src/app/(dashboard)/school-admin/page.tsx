...
import type { AttTrendPoint, RevPoint, AdmPoint, ClassDistPoint } from "@/components/school-admin/dashboard/dashboard-charts";
...
function monthLabel(offsetMonths = 0): string {
  ...
  return t.toLocaleDateString("en-IN", { month: "short" }) + " " + String(t.getFullYear()).slice(2);
}
...
function creationsByMonth(records: { createdAt: Date }[], months: number): number[] {
  const now = new Date();
  const result = new Array(months).fill(0);
  for (const r of records) {
    const ma = (now.getFullYear() - r.createdAt.getFullYear()) * 12 + (now.getMonth() - r.createdAt.getMonth());
    if (ma >= 0 && ma < months) result[months - 1 - ma]++;
  }
  return result;
}

function paymentsByMonth(payments: { paymentDate: Date | null; amountPaid: number }[], months: number): number[] {
  const now = new Date();
  const result = new Array(months).fill(0);
  for (const p of payments) {
    const ref = p.paymentDate ?? null;
    if (!ref) continue;
    const ma = (now.getFullYear() - ref.getFullYear()) * 12 + (now.getMonth() - ref.getMonth());
...
export default async function SchoolAdminPage() {
  ...
  if (!schoolId) {
    return (
      <div className="p-8">
        <p className="text-red-500 text-sm font-medium">No school assigned to your account.</p>
      </div>
    );
  }
  ...
  const [
    ...
    prisma.user.count({ where: { schoolId, role: { in: ["STUDENT", "TEACHER"] }, isActive: true } }),
    ...
    prisma.user.count({ where: { schoolId, role: { in: ["STUDENT", "TEACHER"] }, isActive: true, createdAt: { lt: mStart } } }),

    // Recent user creations for sparklines
    prisma.user.findMany({
      where: { schoolId, role: { in: ["STUDENT", "TEACHER"] }, createdAt: { gte: sevenMonthsAgo } },
      select: { createdAt: true, role: true },
    }),

    // Recent payments for revenue sparklines
    prisma.feePayment.findMany({
      where: { schoolId, amountPaid: { gt: 0 }, paymentDate: { gte: sevenMonthsAgo } },
  ...
  const accountCreations = recentUsers.filter((u) => u.role === "STUDENT" || u.role === "TEACHER");
  ...
  const accountSparkline = creationsByMonth(accountCreations, SPARK_MONTHS);
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dayRows = attendance7Days.filter((r) => new Date(r.date).toISOString().split("T")[0] === key);
  ...
  const attLastMonthRows = attendance7Days.filter((r) => new Date(r.date) < mStart);
  ...
  for (let i = 29; i >= 0; i--) {
    const d = new Date(now.getTime() - i * 86_400_000);
    const key = d.toISOString().split("T")[0]!;
    const dayRows = attendance30Days.filter((r) => new Date(r.date).toISOString().split("T")[0] === key);
  ...
  const dashboardData: DashboardData = {
    ...
    kpis: {
      ...
      parents: {
        ...
        lastMonth: activeAccountLastMonthTotal,
