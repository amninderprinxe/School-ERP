...
import {
  calcGrade, calcPercentage, gradeStyle,
} from "@/lib/results-utils";
import {
  CheckCircle2, AlertCircle, ClipboardCheck, Users,
} from "lucide-react";
...
function GradeBadge({ grade }: { grade: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 text-xs font-bold rounded ${gradeStyle(grade)}`}>
      {grade}
    </span>
  );
}
...
export function ResultsEntryClient({
  ...
  // ── Live stats ─────────────────────────────────────────────────
  const stats = useMemo(() => {
    let marked = 0, totalMarks = 0, totalMax = 0, passed = 0;
    for (const s of students) {
    ...
    const avg = marked > 0 && totalMax > 0
      ? Math.round((totalMarks / totalMax) * 1000) / 10
      : 0;
    const passRate = marked > 0
      ? Math.round((passed / marked) * 100)
      : 0;
  ...
  const handleSectionChange = (val: string) => {
    setLocalSection(val);
    navigate(localExam, val, localSubject);
  };

  const handleSubjectChange = (val: string) => {
    setLocalSubject(val);
    navigate(localExam, localSection, val);
  };
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            {exams.length === 0 ? (
              <p className="text-sm text-amber-600 bg-amber-50 border
          ...
          <div>
            ...
            <select
              ...
              disabled={!localExam || sections.length === 0 || isNavPending}
              className={SEL_INPUT}
            >
              <option value="">
                {!localExam
          ...
          <div>
            ...
            <select
              ...
              disabled={!localExam || subjects.length === 0 || isNavPending}
              className={SEL_INPUT}
            >
              <option value="">
                {!localExam
        ...
        {isNavPending && (
          <div className="mt-3 flex items-center gap-2 text-xs text-blue-600">
            <svg className="animate-spin h-3.5 w-3.5" fill="none"
              viewBox="0 0 24 24">
          ...
          {isUpdate && (
            <div className="flex items-center gap-2.5 p-3.5 bg-blue-50
              ...
              <p className="text-sm text-blue-700">
                Marks are already entered for this selection. Saving will
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Marks Entered",  value: `${stats.marked} / ${students.length}`, color: "text-blue-700" },
                  { label: "Average",        value: `${stats.avg}%`,  color: stats.avg >= 50 ? "text-emerald-700" : "text-red-600" },
                  { label: "Pass Rate",      value: `${stats.passRate}%`, color: stats.passRate >= 50 ? "text-emerald-700" : "text-red-600" },
                ].map((item) => (
                  <div key={item.label}
              ...
              <div className="bg-white rounded-xl border border-gray-100
                ...
                {/* Table header */}
                <div className="grid grid-cols-12 gap-2 px-5 py-3 bg-gray-50
                      ...
                      >
                        ...
                        <div className="col-span-2">
                          <input
                            ...
                            max={9999}
                            step="1"
                            onChange={(e) => updateRowMax(student.id, e.target.value)}
                            className={NUM_INPUT}
                          />
                        </div>
                        ...
                        <div className="col-span-1 text-center">
                          {grade ? (
                            ...
                            <input
                              ...
                              maxLength={3}
                              placeholder="—"
                              onChange={(e) => updateGrade(student.id, e.target.value.toUpperCase())}
              ...
              <div className="flex items-center justify-between py-2">
                <p className="text-xs text-gray-400">
                  {stats.marked} / {students.length} students have marks
                  entered
                  {isUpdate && (
