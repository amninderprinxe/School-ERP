...
import { attendanceAlertEmail, attendanceAlertSubject }
  from "@/lib/email-templates/attendance-alert";
...
export async function saveAttendance(
  ...
  try {
    ...
    // ── EMAIL: notify students of attendance updates ─────────────
    _notifyStudentsOfAttendance({
      entries: entries.filter((e) => e.status !== "PRESENT"),
      sectionId,
      schoolId,
      date,
      markerName: user.name ?? "School",
    });
...
// ── Fire-and-forget: notify students of attendance updates ────────
async function _notifyStudentsOfAttendance(args: {
  entries: { studentProfileId: string; status: string; remarks?: string }[];
  sectionId: string;
  schoolId: string;
  date: string;
  markerName: string;
  ...
  try {
    ...
    const [school, section, students] = await Promise.all([
      ...
      prisma.studentProfile.findMany({
        where: { id: { in: args.entries.map((e) => e.studentProfileId) } },
        include: {
    ...
    const formattedDate = new Date(`${args.date}T00:00:00.000Z`)
    ...
    for (const student of students) {
      const entry = args.entries.find(
        (e) => e.studentProfileId === student.id,
      );
      if (!entry) continue;
      ...
      if (student.user.email) {
        sendEmail({
          ...
          subject: attendanceAlertSubject(student.user.name, entry.status),
          html: attendanceAlertEmail({
        ...
        createNotification({
          ...
          title: `${student.user.name} was marked ${entry.status === "ABSENT" ? "Absent" :
              entry.status === "LATE" ? "Late" :
                entry.status === "HALF_DAY" ? "Half Day" : entry.status
            } today`,
          body: `Date: ${formattedDate}. Marked by ${args.markerName}.${entry.remarks ? ` Note: ${entry.remarks}` : ""
            }`,
