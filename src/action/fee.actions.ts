...
import { $Enums, PaymentMode, Prisma } from "@prisma/client";
...
export async function recordPayment(
  ...
  try {
    ...
    // ── NOTIFY student ────────────────────────────────────────────────
    const studentUser = await prisma.studentProfile
      .findFirst({
    ...
    if (studentUser) {
      const outstanding = Math.max(
        0,
        structure.amount - amountPaid - (parsed.data.waivedAmount ?? 0),
      );
      createNotification({
        userId: studentUser.user.id,
        schoolId,
        title: `Fee payment recorded — ${structure.feeCategory?.name ?? "Fee"}`,
        body: outstanding > 0
          ? `₹${amountPaid.toLocaleString("en-IN")} paid. Balance: ₹${outstanding.toLocaleString("en-IN")}.`
          : `₹${amountPaid.toLocaleString("en-IN")} paid. Fully settled ✓`,
        link: "/student/fees",
      });
    }
...
async function sendFeeConfirmationEmail(
  ...
  if (!apiKey) {
    console.warn(
      "[sendFeeConfirmationEmail] SENDGRID_API_KEY is not configured.",
    );
    return;
  }
  ...
  const fromHost = appUrl
    .replace(/^https?:\/\//, "")
    .replace(/\/$/, "")
    .replace(/:\d+$/, "");
  const fromEmail =
    process.env.EMAIL_FROM_ADDRESS ?? `no-reply@${fromHost}`;
  ...
  const outstandingFormatted =
    payload.outstanding.toLocaleString("en-IN", {
      style: "currency",
      currency: "INR",
    });
  const paymentDateFormatted =
    payload.paymentDate.toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });

  const subject = `Fee payment confirmation for ${payload.studentName}`;
  const text = `Hello ${payload.recipientName},

Your fee payment has been recorded for ${payload.studentName}.

School: ${payload.schoolName}
Fee: ${payload.categoryName}
Academic Year: ${payload.academicYear}
Amount Paid: ${amountFormatted}
Outstanding: ${outstandingFormatted}
Status: ${payload.status}
Payment Mode: ${payload.paymentMode}
Transaction Reference: ${payload.transactionRef || "N/A"}
Payment Date: ${paymentDateFormatted}

You can view the fee details by logging in:
${payload.loginUrl}

Thank you.
`;

  const html = `<p>Hello ${payload.recipientName},</p>
<p>Your fee payment has been recorded for <strong>${payload.studentName}</strong>.</p>
<ul>
  <li><strong>School:</strong> ${payload.schoolName}</li>
  <li><strong>Fee:</strong> ${payload.categoryName}</li>
  <li><strong>Academic Year:</strong> ${payload.academicYear}</li>
  <li><strong>Amount Paid:</strong> ${amountFormatted}</li>
  <li><strong>Outstanding:</strong> ${outstandingFormatted}</li>
  <li><strong>Status:</strong> ${payload.status}</li>
  <li><strong>Payment Mode:</strong> ${payload.paymentMode}</li>
  <li><strong>Transaction Reference:</strong> ${payload.transactionRef || "N/A"}</li>
  <li><strong>Payment Date:</strong> ${paymentDateFormatted}</li>
</ul>
<p><a href="${payload.loginUrl}">Login to view details</a></p>
<p>Thank you.</p>`;
  ...
  try {
    const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        personalizations: [
          {
            to: [{ email, name: payload.recipientName }],
            subject,
          },
        ],
        from: {
    ...
    if (!response.ok) {
      ...
      console.error(
        "[sendFeeConfirmationEmail] Failed to send email.",
        response.status,
        response.statusText,
        errorText,
      );
    ...
    console.error("[sendFeeConfirmationEmail]", error);
  }
}
