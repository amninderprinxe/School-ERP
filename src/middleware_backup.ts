...
const ROLE_DASHBOARD: Record<Exclude<Role, "PARENT">, string> = {
...
const ROLE_ALLOWED_PREFIXES: Record<Exclude<Role, "PARENT">, string[]> = {
  SUPER_ADMIN:  ["/super-admin"],
  SCHOOL_ADMIN: ["/school-admin"],
  TEACHER:      ["/teacher"],
  STUDENT:      ["/student"],
...
export default auth((req) => {
  ...
  if (isPublic && isAuthenticated) {
    const dest = ROLE_DASHBOARD[session.user.role as keyof typeof ROLE_DASHBOARD ];
  ...
  if (isAuthenticated) {
    ...
    const allowed = ROLE_ALLOWED_PREFIXES[role as keyof typeof ROLE_ALLOWED_PREFIXES];
    ...
    if (!isAllowed) {
      return NextResponse.redirect(
        new URL(ROLE_DASHBOARD[role as keyof typeof ROLE_DASHBOARD], req.url),
      );
    }
  }
